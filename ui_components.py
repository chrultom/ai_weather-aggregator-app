...
MODEL_PALETTE = ["#2563EB", "#059669", "#7C3AED", "#EA580C", "#D97706", "#0891B2", "#DC2626"]
...
def plot_master_consensus(
    ...
    fig.update_layout(
        title=dict(text=f"Temperature Consensus & Multi-Model Forecast \u2014 {city}", font=dict(size=18)),
        xaxis=dict(title="Date", gridcolor="#F3F4F6", tickformat="%a, %b %d", showgrid=True),
...
def plot_rain_forecast(df_rain: pd.DataFrame, df_summary: pd.DataFrame, city: str) -> go.Figure:
    ...
    fig.update_layout(
        title=dict(text=f"Daily Precipitation (Rainfall) Forecast by Model \u2014 {city}", font=dict(size=18)),
        xaxis=dict(title="Date", gridcolor="#F3F4F6", tickformat="%a, %b %d", showgrid=True),
        yaxis=dict(title="Daily Precipitation (mm)", gridcolor="#F3F4F6", rangemode="tozero"),
        barmode="group",
        hovermode="x unified",
        plot_bgcolor="#FFFFFF",
        paper_bgcolor="#FFFFFF",
...
def plot_cloud_forecast(df_cloud: pd.DataFrame, df_summary: pd.DataFrame, city: str) -> go.Figure:
    ...
    fig.add_hrect(y0=0, y1=20, fillcolor="#FEF9C3", opacity=0.35, layer="below", line_width=0, annotation_text="\u2600\ufe0f Sunny / Clear (<20%)", annotation_position="top left")
    fig.add_hrect(y0=20, y1=60, fillcolor="#F1F5F9", opacity=0.35, layer="below", line_width=0, annotation_text="\u26c5 Partly Cloudy (20\u201360%)", annotation_position="top left")
    fig.add_hrect(y0=60, y1=85, fillcolor="#E2E8F0", opacity=0.35, layer="below", line_width=0, annotation_text="\U0001f325\ufe0f Mostly Cloudy (60\u201385%)", annotation_position="top left")
    fig.add_hrect(y0=85, y1=100, fillcolor="#CBD5E1", opacity=0.35, layer="below", line_width=0, annotation_text="\u2601\ufe0f Overcast (>85%)", annotation_position="top left")
    ...
    fig.update_layout(
        title=dict(text=f"Daily Cloud Cover / Cloudiness Forecast (%) \u2014 {city}", font=dict(size=18)),
        xaxis=dict(title="Date", gridcolor="#F3F4F6", tickformat="%a, %b %d", showgrid=True),
        yaxis=dict(title="Cloud Cover (%)", range=[0, 105], gridcolor="#F3F4F6", dtick=20),
        hovermode="x unified",
        plot_bgcolor="#FFFFFF",
        paper_bgcolor="#FFFFFF",
        legend=dict(orientation="h", yanchor="bottom", y=-0.35, xanchor="center", x=0.5),
...
def plot_temp_comparison(df: pd.DataFrame, ensemble_series: pd.Series, title: str, unit: str) -> go.Figure:
...
def plot_spread(df_summary: pd.DataFrame, unit: str, city: str) -> go.Figure:
    ...
    fig.update_layout(
        ...
        xaxis=dict(title="Date", gridcolor="#F3F4F6", tickformat="%a, %b %d"),
        yaxis=dict(title=f"Spread ({unit})", gridcolor="#F3F4F6"),
        plot_bgcolor="#FFFFFF",
        paper_bgcolor="#FFFFFF",
